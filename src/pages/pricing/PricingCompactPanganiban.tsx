...
function PricingCard(props: { option: PricingOption; isDark: boolean }) {
  ...
  const scanlineColor = 'amber-500'

  return (
    <div class="backdrop-blur-sm clip-corner overflow-hidden group hover:scale-[1.02] transition-all"
      style={{
        background: props.isDark ? 'linear-gradient(135deg, #0f0f0f 0%, #1a1a1a 100%)' : 'linear-gradient(135deg, #ffffff 0%, #f9fafb 100%)',
        border: props.isDark ? '1px solid rgba(39, 39, 42, 0.5)' : '1px solid rgba(209, 213, 219, 0.5)'
      }}
    >
      {/* Scan-line effect */}
      <div class={`absolute inset-0 bg-gradient-to-r from-transparent via-${scanlineColor}/5 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000`} />
      ...
      <div class="relative z-10">
        ...
        <Show when={props.option.coverImage}>
          <div class="relative h-48 overflow-hidden rounded-t-lg">
            ...
            <div class="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-black/90 to-transparent pointer-events-none"></div>
          </div>
        </Show>
        ...
        <div class="p-3">
          ...
          <div class={`flex items-center gap-4 ${props.option.coverImage ? 'px-6' : 'px-6'} mb-8`}>
            <div class="p-3 rounded-lg" style={{background: props.isDark ? 'rgba(39, 39, 42, 0.5)' : 'rgba(243, 244, 246, 0.8)'}}>
              <div style={{color: props.isDark ? '#a1a1aa' : '#52525b'}}><Icon size={20} /></div>
            </div>
            <div>
              <h3 class="text-lg font-bold mb-1" style={{color: props.isDark ? '#ffffff' : '#18181b'}}>{props.option.name}</h3>
            </div>
          </div>
              ...
              <div class="relative w-[60%] max-w-[260px]">
                ...
                >
                  <div class="flex flex-col items-center justify-center h-full pr-2 pb-2">
                    <div class="text-xs uppercase tracking-wider mb-1 font-bold" style={{color: props.isDark ? '#a1a1aa' : '#71717a'}}>Org Partner</div>
                    <div class={props.isDark ? 'gradient-text' : 'gradient-text-light'} style={{'font-size': '1.875rem', 'font-weight': 'bold', 'line-height': '2.25rem'}}>
          ...
          <div class={props.option.additionalPricing.length === 1 ? "space-y-2" : "grid grid-cols-1 sm:grid-cols-2 gap-2"}>
            {props.option.additionalPricing.map((pricing) => {
              ...
              return (
                <div
                  class={`${isSingleItem ? 'flex items-center justify-between p-4' : 'flex flex-col justify-between p-3'} rounded-lg border border-zinc-700/50 hover:bg-zinc-800/60 hover:border-zinc-600/30 transition-all ${pricingType === PricingType.EXTENSION ? 'bg-gradient-to-r from-amber-400/20 via-zinc-800/40 to-zinc-800/30' :
                    pricingType === PricingType.LEGACY ? 'bg-gradient-to-r from-zinc-800/30 via-zinc-800/40 to-zinc-500/20' :
                      'bg-zinc-800/30'
                    }`}
                >
                  {isSingleItem ? (
                    // Single item layout (original style)
                    <>
                      <span class={`${pricingType === PricingType.LEGACY ? 'text-green-300' : 'text-zinc-300'} text-sm ${pricingType !== PricingType.NONE ? 'font-bold' : 'font-medium'}`}>
                      ...
                      <div class="flex items-center gap-4">
                        ...
                        <div class="w-px h-10 bg-zinc-700/50"></div>
                        <div class="text-right">
                    ...
                    // Multi-item layout (2-column grid style)
                    <>
...
export default function PricingCompactPanganiban() {
  ...
  return (
    <>
      ...
      <section class="relative w-full" style={{ background: isDark() ? 'linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%)' : 'linear-gradient(135deg, #f9fafb 0%, #f3f4f6 100%)' }}>
        {/* Background patterns - Full viewport width */}
        ...
        <div class="relative mt-20 z-10" style={{ 'padding-top': '2rem', 'padding-bottom': '4rem' }}>
          ...
          <div class="flex items-center justify-center gap-4 mb-6">
            ...
            />
            <h2 class="text-4xl md:text-5xl font-bold" style={{color: isDark() ? '#ffffff' : '#18181b'}}>
          ...
          <div class="flex justify-center gap-8 mt-8 text-sm">
            <div class="flex items-center gap-2">
              ...
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>Starts at 4 hours</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-amber-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>Hourly extensions</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-blue-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>Free unlimited coffee</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-purple-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>High-speed internet</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-amber-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>Power at each table</span>
            </div>
          </div>

          {/* Last Updated Notice */}
          <div class="text-center mt-6">
            <div class="text-sm" style={{color: isDark() ? '#71717a' : '#a1a1aa'}}>
      ...
      <section class="relative w-full" style={{ background: isDark() ? 'linear-gradient(135deg, #151515 0%, #1f1f1f 100%)' : 'linear-gradient(135deg, #f3f4f6 0%, #e5e7eb 100%)' }}>
        {/* Background patterns for premium section - Full viewport width */}
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div class="relative z-10" style={{ 'padding-top': '4rem', 'padding-bottom': '4rem' }}>
          {/* Section Divider */}
          <div class="absolute left-1/2 top-0 -translate-x-1/2 w-32 h-px bg-gradient-to-r from-transparent via-zinc-600 to-transparent"></div>
          ...
          <div class="flex items-center justify-center gap-4 mb-4">
            <img
              ...
              class="w-30 object-contain"
            />
            <h3 class="text-3xl md:text-4xl font-bold" style={{color: isDark() ? '#ffffff' : '#18181b'}}>
          ...
          <div class="flex justify-center gap-8 mt-6 text-sm">
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-purple-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>24/7 biometric access</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-blue-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>Private event space</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full bg-green-400"></div>
              <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>No front desk needed</span>
            </div>
          </div>
        </div>
      ...
      <section class="relative w-full" style={{ background: isDark() ? 'linear-gradient(135deg, #1a1a1a 0%, #252525 100%)' : 'linear-gradient(135deg, #e5e7eb 0%, #d1d5db 100%)' }}>
        {/* Background patterns - Full viewport width */}
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div class="relative z-10" style={{ 'padding-top': '4rem', 'padding-bottom': '4rem' }}>
          ...
          <div class="max-w-4xl mx-auto text-center mb-12">
            ...
            <div class="flex justify-center gap-8 mt-6 text-sm">
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full bg-green-400"></div>
                <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>20% discount for members</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full bg-amber-400"></div>
                <span style={{color: isDark() ? '#a1a1aa' : '#71717a'}}>Valid organization ID required</span>
              </div>
            </div>
          </div>
