...
async function captureMonthlyUpdateP2Screenshot(
  page: any,
  mode: 'light' | 'dark',
  outputPath: string
) {
  const url = mode === 'dark'
    ? '/print/monthly-update/oct-2025-p2?dark'
    : '/print/monthly-update/oct-2025-p2'

  await page.goto(url)
  await page.waitForLoadState('networkidle')
  await page.waitForSelector('text=Monthly Update')
  await page.waitForSelector('img[alt="KahitSan Logo"]')
  await page.waitForSelector('img[alt="UAPGA CAMARINES CHAPTER Logo"]')
...
test.describe('Monthly Update Part 2 Print Page Screenshot', () => {
  test.beforeAll(() => {
  ...
  test('capture Light Mode - Monthly Update P2', async ({ page }) => {
    await captureMonthlyUpdateP2Screenshot(page, 'light', 'screenshots/monthly-update/light/monthly-update-oct-2025-p2.png')
  })

  test('capture Dark Mode - Monthly Update P2', async ({ page }) => {
    await captureMonthlyUpdateP2Screenshot(page, 'dark', 'screenshots/monthly-update/dark/monthly-update-oct-2025-p2.png')
  })
})
